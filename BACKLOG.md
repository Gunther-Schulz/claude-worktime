...
- 2026-09-13: **session name in the statusline — SHIPPED `fc22fe8`**
  ...
  discriminator answered YES by the dispatching desk from a raw
  one-render dump of the live payload (tee shim on the statusLine
  command, keys read, config restored byte-identical, dump deleted):
  the stdin JSON carries top-level `session_name` (string). Built as
  the `{session_name}` token (`{session}` was taken — active-duration
  token), in STATUSLINE_3's default beside MODEL; absent key and
  empty string both drop the segment cleanly. Suite 28/28 including
  the new synthesized-fixture test (public-repo constraint honored:
  no real payload, no real name). Deployed same hour: script copied
  to `~/.local/bin/claude-worktime`; the operator's config overrides
  STATUSLINE_3, so the token was added there too (dotfiles
  `5b25078`, landed by the desk holding that copy).
