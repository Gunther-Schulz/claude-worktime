...
| `ctx 77%` | Context window fullness; `❄397k·other 2m` appended for the last cold-cache rewrite — size·cause and age, cyan when recent, gray once old |
...
| `{context}` | Context window usage (e.g. `77%`); appends `❄397k·other 2m` for the most recent cold-cache rewrite — size·cause and age (cyan when recent, gray after `COLD_FRESH_SECS`); hidden until the first |
