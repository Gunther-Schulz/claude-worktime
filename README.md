...
**The residual is two classes, not one.** When no diagnostic explains the rewrite, the cause is `no-prefix` if the request read *nothing* from cache (`cr == 0`) and plain `other` otherwise. These are different events: a total miss matched no cached prefix at all and re-wrote the whole context, while a partial miss hit a prefix and re-billed only the remainder. Measured 2026-08-13 over ten cold rows with perfect separation in both directions — all seven `cr == 0` rows had a null API cause, all three `cr > 0` rows had a real one — and the control is built into the same run: the identical reader over the identical file returned three populated causes, so the absence is real rather than a reader failure. `other` therefore now means what its name always implied: a cause could have existed and none was available. **`no-prefix` names an observable and asserts no mechanism** — *why* a total miss happens is unsettled, and the leading hypothesis was measured and refuted on 2026-08-13; a name implying a cause would be the same label-over-body problem the split exists to end. Classification happens when the event is written, so rows logged before this split keep the `other` label they were recorded with.
...
**How an agent's state is read:** from the *last* conversation record of its log. A tool call in flight, a thinking-only record, or a user record (a tool result the model is answering) means **busy** — named, with its quiet time. A closing text reply means **idle** for a teammate, which waits for its next message and is counted (`3 idle`), and **done** for an Agent-tool agent, whose result has already reached you — not shown. Claude Code's own interruption marker (`[Request interrupted by user]`, or its `for tool use` variant) means **stopped** — not shown. The log carries no structured flag for an interruption; that marker is Claude Code's own signal, and it is matched with the same pattern Claude Code itself uses, so a prompt that merely begins with those words is not mistaken for one. That rule was measured over 1,179 real agent logs (2026-09-11); the basis sits at the top of `tests/statusline-agents.sh`. Busy agents are listed longest-quiet first, so the one most likely stuck leads.
