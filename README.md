...
| `ctx 77%` | Context window fullness; `❄130k` appended with the size of the last cold-cache rewrite this session |
...
| `{context}` | Context window usage (e.g. `77%`); appends `❄130k` — the size of the most recent cold-cache rewrite this session (hidden until the first) |
